...
export class TerminalLifecycleManager {
  ...
  private linkProviderDisposables = new Map<string, IDisposable>();

  private readonly absoluteFilePathRegex = /(?:[A-Za-z]:)?[\\/][^\s'"`]+?(?:\.[^\s'"`]+)?(?:\:\d+){0,2}/g;
  private readonly relativeFilePathRegex = /(?:\.\.\/[\\/]|\.\/[\\/]|)(?:[\w.-]+[\\/])*[\w.-]+\.[\w]+(?:\:\d+){0,2}/g;
      ...
      try {
        ...
        const webLinksAddon = new WebLinksAddon((event: MouseEvent, uri: string) => {
          try {
            ...
            terminalLogger.warn('⚠️ Failed to prevent default web link click:', error);
          }
        ...
        // Register link handlers (URLs and file paths)
        this.registerTerminalLinkHandlers(terminal, terminalId);
  ...
  public async removeTerminal(terminalId: string): Promise<boolean> {
    try {
      ...
      const linkDisposable = this.linkProviderDisposables.get(terminalId);
      if (linkDisposable) {
