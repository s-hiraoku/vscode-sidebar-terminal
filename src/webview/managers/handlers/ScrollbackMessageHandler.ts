...
export class ScrollbackMessageHandler implements IMessageHandler {
  ...
  private handleRestoreScrollback(
    ...
    this.logger.debug('[SCROLLBACK-RESTORE] handleRestoreScrollback called', {
    ...
    try {
      ...
      this.logger.debug(
        `[SCROLLBACK-RESTORE] ✅ Restored ${normalizedScrollback.length} lines to terminal ${terminalId}`
      );
      this.logger.info(
  ...
  private async handleExtractScrollbackData(
    ...
    try {
      ...
      this.logger.info(
        `📦 [SAVE-DEBUG] Extracting scrollback data for terminal ${terminalId}, requestId: ${requestId}`
      );
      ...
      if (!terminalInstance) {
        this.logger.error(
          `❌ [SAVE-DEBUG] Terminal ${terminalId} not found for scrollback extraction`
        );
  ...
  private restoreScrollbackToXterm(terminal: Terminal, scrollbackContent: ScrollbackLine[]): void {
    this.logger.debug(
      `[SCROLLBACK-RESTORE] restoreScrollbackToXterm: ${scrollbackContent.length} lines`
    );
    ...
    this.logger.debug(
      `[SCROLLBACK-RESTORE] ✅ Finished writing ${scrollbackContent.length} lines to terminal`
    );
  ...
  private extractScrollbackFromTerminal(terminal: any, maxLines: number): string[] {
    try {
      ...
      if (xtermInstance.buffer && xtermInstance.buffer.active) {
        ...
        this.logger.info(`Extracted ${lines.length} lines from terminal buffer (plain text)`);
        return lines;
      }
  ...
  private async handleRestoreTerminalSessions(
    ...
    for (const terminalData of terminals) {
      if (
        ...
        terminalData.restoreScrollback &&
        terminalData.scrollbackData?.length
      ) {
        TerminalCreationService.markTerminalRestoring(terminalData.terminalId);
      }
    }
    this.logger.debug(`[SCROLLBACK-RESTORE] Processing ${terminals.length} terminals`);
    ...
    this.logger.debug(
      `[SCROLLBACK-RESTORE] ✅ Completed restoration: ${successCount} success, ${failedCount} failed`
    );
    this.logger.info(
