...
class TerminalWebviewManager {
  ...
  private maxSplitCount = 2; // Limit to 2 terminals for stability
  ...
  private initializeMultiSplitLayout(): void {
    ...
    if (existingPrimaryTerminal) {
      ...
      // Resize the terminal instance to fit new container
...
class StatusManager {
  ...
  private adjustSplitContainersHeight(availableHeight: number): void {
    ...
    if (splitContainers.length > 0) {
      ...
      const heightPerTerminal = Math.floor(availableHeight / splitCount);

      console.log(`📐 [LAYOUT] Adjusting ${splitCount} terminals to ${heightPerTerminal}px each (total: ${availableHeight}px)`);

      splitContainers.forEach((container) => {
        (container as HTMLElement).style.height = `${heightPerTerminal}px`;
      });
    }
  }
