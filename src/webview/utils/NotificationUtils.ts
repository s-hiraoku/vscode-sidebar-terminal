...
export class NotificationUtils {
   ...
   * ターミナル削除エラーの表示
   */
  public static showTerminalCloseError(minCount: number): void {
   ...
   * ターミナルキルエラーの表示
   */
  public static showTerminalKillError(reason: string): void {
    this.showNotification({
      type: 'error',
   ...
   * 分割制限警告の表示
   */
  public static showSplitLimitWarning(reason: string): void {
    this.showNotification({
      type: 'warning',
   ...
   * 汎用的な通知表示
   */
  public static showNotification(config: NotificationConfig): void {
   ...
   * 通知要素の作成
   */
  private static createNotificationElement(config: NotificationConfig): HTMLElement {
  ...
  private static removeNotification(notification: HTMLElement): void {
    if (notification.parentNode && this.activeNotifications.has(notification)) {
