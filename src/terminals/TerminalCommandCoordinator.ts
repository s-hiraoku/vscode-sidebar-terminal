...
import { getTerminalConfig, ActiveTerminalManager, getFirstValue, showErrorMessage } from '../utils/common';
 ...
 *
 * Consolidated coordinator that combines:
 * - TerminalStateCoordinator: State management and synchronization
 * - TerminalIOCoordinator: Input/output operations
 *
 * Responsibility: Unified terminal state and I/O operations
 * - Track and manage terminal state
 * - Handle terminal input/output
 * - Manage activation/deactivation
 * - PTY communication and recovery
 *
 * Consolidation Benefits:
 * - Single source of truth for terminal state + operations
 * - Cleaner dependency graph
 * - Reduced inter-coordinator communication overhead
 */
export class TerminalCommandCoordinator {
  ...
  // ==========================================
  // STATE MANAGEMENT (from TerminalStateCoordinator)
  // ==========================================
  ...
  // ==========================================
  // I/O OPERATIONS (from TerminalIOCoordinator)
  // ==========================================
  ...
  private writeToPtyWithValidation(
    ...
    if (!ptyInstance) {
      ...
      if (retryAttempt >= TerminalCommandCoordinator.MAX_PTY_RETRY_ATTEMPTS) {
        log(
      ...
      log(
        `⏳ [PTY-WAIT] PTY not ready for terminal ${terminal.id}, queuing input (attempt ${retryAttempt + 1}/${TerminalCommandCoordinator.MAX_PTY_RETRY_ATTEMPTS})...`
      );

      // 🔧 FIX: Queue the input and try again with exponential backoff
      const delay = TerminalCommandCoordinator.PTY_RETRY_DELAY_MS * Math.pow(1.5, retryAttempt);
      setTimeout(() => {
