...
describe('WebViewMessageRoutingService', () => {
  ...
  describe('Message Routing', () => {
    ...
    it('should propagate handler errors', async () => {
      ...
      await assert.rejects(service.routeMessage(message, mockContext), /Handler failure/);
    });
...
describe('InitializationMessageHandler', () => {
  ...
  it('should create initial terminal when requested and none exist', async () => {
    ...
    assert.ok(
      (mockContext.terminalManager.setActiveTerminal as sinon.SinonStub).calledWith('terminal-1')
    );
...
describe('TerminalControlMessageHandler', () => {
  ...
  it('should handle input command with data', async () => {
    ...
    assert.ok(
      (mockContext.terminalManager.sendInput as sinon.SinonStub).calledWith(
        'terminal-1',
        'test input'
      )
    );
  ...
  it('should handle resize command with dimensions', async () => {
    ...
    assert.ok(
      (mockContext.terminalManager.resizeTerminal as sinon.SinonStub).calledWith(
        'terminal-1',
        80,
        24
      )
...
describe('TerminalManagementMessageHandler', () => {
  ...
  it('should handle createTerminal command', async () => {
    ...
    assert.ok((mockContext.terminalManager.createTerminal as sinon.SinonStub).calledOnce);
    assert.ok(
      (mockContext.terminalManager.getTerminalById as sinon.SinonStub).calledWith('new-terminal')
    );
  });
  ...
  it('should handle focusTerminal command', async () => {
    ...
    assert.ok(
      (mockContext.terminalManager.setActiveTerminal as sinon.SinonStub).calledWith(
        'target-terminal'
      )
    );
  });
  ...
  it('should handle terminalClosed command when terminal exists', async () => {
    mockContext.terminalManager.getTerminals = sandbox
      .stub()
      .returns([{ id: 'existing-terminal' }]);
    const message: WebviewMessage = {
    ...
    assert.ok(
      (mockContext.terminalManager.removeTerminal as sinon.SinonStub).calledWith(
        'existing-terminal'
      )
    );
  });
  ...
  it('should handle killTerminal command with specific terminal ID', async () => {
    ...
    assert.ok(
      (mockContext.killSpecificTerminal as sinon.SinonStub).calledWith('specific-terminal')
    );
  });
  ...
  it('should handle deleteTerminal command', async () => {
    ...
    assert.ok(
      (mockContext.deleteTerminalUnified as sinon.SinonStub).calledWith('delete-terminal', 'header')
    );
  });
  ...
  it('should default to panel source for deleteTerminal', async () => {
    ...
    assert.ok(
      (mockContext.deleteTerminalUnified as sinon.SinonStub).calledWith('delete-terminal', 'panel')
    );
  });
});
...
describe('Integration Tests', () => {
  ...
  it('should handle complete message flow for terminal operations', async () => {
    ...
    assert.ok(
      (mockContext.terminalManager.sendInput as sinon.SinonStub).calledWith('terminal-1', 'ls -la')
    );
    ...
    assert.ok(
      (mockContext.deleteTerminalUnified as sinon.SinonStub).calledWith('terminal-1', 'header')
    );
  });
