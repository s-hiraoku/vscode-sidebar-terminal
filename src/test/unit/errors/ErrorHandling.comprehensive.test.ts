...
describe('Comprehensive Error Handling Tests (TDD)', () => {
  ...
  describe('RED Phase: Error Handling Specifications', () => {
    describe('Error classification', () => {
      it('should classify errors by type', () => {
        ...
        expect(error.recoverable).to.be.a('boolean');
      });
      ...
      it('should distinguish between recoverable and fatal errors', () => {
        ...
        expect(recoverableError.recoverable).to.be.true;
        expect(fatalError.recoverable).to.be.false;
      });
    });
    ...
    describe('Error propagation', () => {
      it('should propagate errors through call stack', () => {
        ...
        expect(wrapped.cause).to.equal(error);
        expect(wrapped.context).to.be.a('string');
      });
  ...
  describe('GREEN Phase: Basic Error Handling Implementation', () => {
    describe('Error creation and classification', () => {
      it('should create typed error objects', () => {
        ...
        expect(error.type).to.equal('TERMINAL_NOT_FOUND');
        expect(error.message).to.equal('Terminal not found');
      ...
      it('should classify terminal errors', () => {
        ...
        expect(errors.TERMINAL_NOT_FOUND.recoverable).to.be.true;
        expect(errors.TERMINAL_CREATION_FAILED.recoverable).to.be.false;
      });
    });
    ...
    describe('Error handling in terminal operations', () => {
      ...
      it('should handle terminal creation failure', () => {
        ...
        try {
          ...
          expect((error as Error).message).to.equal('PTY creation failed');
        }
      });
      ...
      it('should handle terminal process crash', () => {
        ...
        expect(terminal.process.crashed).to.be.true;
      });
    });
    ...
    describe('Error recovery mechanisms', () => {
      ...
      it('should implement exponential backoff', () => {
        ...
        expect(backoffDelays).to.deep.equal(delays);
      });
    });
  });
  ...
  describe('REFACTOR Phase: Enhanced Error Handling', () => {
    describe('Structured error handling', () => {
      it('should use Result pattern for error handling', () => {
        ...
        if (result.success) {
          expect(result.value.id).to.equal('terminal-1');
        }
      });
      ...
      it('should chain error contexts', () => {
        ...
        expect(fullContext).to.include('In function B');
        expect(fullContext).to.include('In function A');
      });
    });
    ...
    describe('Error recovery strategies', () => {
      it('should implement graceful degradation', () => {
        ...
        expect(result).to.equal('basic feature');
      });
    ...
    describe('Error monitoring and reporting', () => {
      it('should log errors with context', () => {
        ...
        expect(errorLog).to.have.length(1);
        expect(errorLog[0].message).to.equal('Test error');
        expect(errorLog[0].context.terminalId).to.equal('test-1');
      });
      ...
      it('should aggregate error statistics', () => {
        ...
        expect(errorStats.total).to.equal(3);
        expect(errorStats.byType.get('TERMINAL_NOT_FOUND')).to.equal(2);
        expect(errorStats.byType.get('NETWORK_ERROR')).to.equal(1);
      });
    });
  });
    ...
    describe('Terminal lifecycle errors', () => {
      it('should handle error during terminal creation', () => {
        ...
        try {
          ...
          expect((error as Error).message).to.include('PTY initialization failed');
        }
      });
      ...
      it('should handle error during terminal disposal', () => {
        ...
        try {
          ...
          expect((error as Error).message).to.include('Process kill failed');
      ...
      it('should handle terminal process unexpected exit', () => {
        ...
        expect(events).to.include('exit');
        expect(events).to.include('exit-code-1');
      });
    });
    ...
    describe('WebView communication errors', () => {
      it('should handle WebView message timeout', async () => {
        ...
        try {
          ...
          expect((error as Error).message).to.equal('Message timeout');
        }
      });
      ...
      it('should handle WebView not ready', () => {
        ...
        try {
          ...
          expect((error as Error).message).to.equal('WebView not ready');
        }
      });
      ...
      it('should handle message serialization error', () => {
        ...
        try {
          ...
          expect((error as Error).message).to.include('circular');
        }
      });
    });
    ...
    describe('Configuration errors', () => {
      ...
      it('should handle configuration update errors', () => {
        ...
        try {
          ...
          expect((error as Error).message).to.equal('Invalid value type');
        }
      });
    });
    ...
    describe('Resource errors', () => {
      ...
      it('should handle file descriptor exhaustion', () => {
        ...
        try {
          ...
          expect((error as Error).message).to.equal('Too many open files');
        }
      });
    });
  });
  ...
  describe('Error Recovery Best Practices', () => {
    ...
    it('should implement transaction rollback', () => {
      ...
      expect(database.items.size).to.equal(0);
    });
    ...
    it('should implement compensation for partial failures', () => {
      ...
      expect(operations.step1Done).to.be.false;
    });
  });
});
