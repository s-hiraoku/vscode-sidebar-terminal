 ...
 * Tests the complete Claude Code integration feature including status display and performance optimization
 */
...
import { DOMManager } from '../../../webview/managers/DOMManager';
...
describe('Claude Code Integration', () => {
  ...
  describe('Claude Status Display', () => {
    beforeEach(() => {
      // Create test terminal headers
      const container = document.getElementById('terminal-container')!;
      const header1 = domManager.createTerminalHeader('term1', 'Terminal 1');
      const header2 = domManager.createTerminalHeader('term2', 'Terminal 2');
      container.appendChild(header1.element);
      container.appendChild(header2.element);
    });

    it('should update Claude status for active terminal', () => {
      // Act
      uiManager.updateClaudeStatusDisplay('Terminal 1', 'connected');
      ...
      expect(status1?.textContent).to.equal('Claude Code connected');
    ...
    it('should show disconnected status correctly', () => {
      // Act
      uiManager.updateClaudeStatusDisplay('Terminal 1', 'disconnected');
      ...
      expect(status?.textContent).to.equal('Claude Code disconnected');
    ...
    it('should clear status when none is specified', () => {
      ...
      // Act - clear status
      uiManager.updateClaudeStatusDisplay('Terminal 1', 'none');
  ...
  describe('Performance Optimization', () => {
    ...
    it('should use faster flush interval in Claude Code mode', () => {
      // Setup
      performanceManager.setClaudeCodeMode(true);
      const scheduleStub = sinon.stub(global.window, 'setTimeout');

      // Act
      performanceManager.scheduleOutputBuffer('test', mockTerminal);

      // Assert - should use 4ms interval for Claude Code mode
  ...
  describe('Message Processing', () => {
    it('should handle Claude status update message', () => {
      // Setup
      const updateSpy = sinon.spy(uiManager, 'updateClaudeStatusDisplay');
      
      // Create test headers
      const container = document.getElementById('terminal-container')!;
      const header = domManager.createTerminalHeader('term1', 'Terminal 1');
  ...
  describe('DOM Manager', () => {
    it('should create terminal header with correct structure including close button', () => {
      // Act
      const header = domManager.createTerminalHeader('test-id', 'Test Terminal');

      // Assert
      expect(header.element.getAttribute('data-terminal-id')).to.equal('test-id');
      expect(header.element.className).to.include('terminal-header');
      expect(header.nameSpan.textContent).to.equal('Test Terminal');
      expect(header.nameSpan.className).to.include('terminal-name');
      expect(header.statusSpan.className).to.include('claude-status');
      expect(header.indicator.className).to.include('claude-indicator');
      expect(header.closeButton.textContent).to.equal('×');
      expect(header.closeButton.className).to.include('close-btn');
      
      // Verify close button is positioned at the rightmost
      const lastChild = header.element.lastElementChild;
      expect(lastChild).to.equal(header.closeButton);
    });
    ...
    it('should update Claude status with correct formatting', () => {
      ...
      // Act
      const success = domManager.updateClaudeStatus(header.element, 'Terminal 1', 'connected');

      // Assert
      expect(success).to.be.true;
      const statusSpan = header.element.querySelector('.claude-status');
      const indicator = header.element.querySelector('.claude-indicator') as HTMLElement;
      
      expect(statusSpan?.textContent).to.equal('Claude Code connected');
      expect(indicator?.textContent).to.equal('●');
    ...
    it('should preserve close button and keep it at rightmost position when updating Claude status', () => {
      ...
      const statusSpan = header.element.querySelector('.claude-status');
      const indicator = header.element.querySelector('.claude-indicator');
      const closeBtn = header.element.querySelector('.close-btn');
    ...
    it('should remove only Claude status elements when clearing status and preserve close button position', () => {
      ...
      domManager.updateClaudeStatus(header.element, 'Terminal 1', 'connected');
      
      // Verify status was added
      expect(header.element.querySelector('.claude-status')).to.not.be.null;
      expect(header.element.querySelector('.claude-indicator')).to.not.be.null;

      // Act - clear Claude status
      domManager.updateClaudeStatus(header.element, 'Terminal 1', 'none');

      // Assert - Claude status removed but close button preserved at rightmost position
      expect(header.element.querySelector('.claude-status')).to.be.null;
      expect(header.element.querySelector('.claude-indicator')).to.be.null;
      expect(header.element.querySelector('.close-btn')?.textContent).to.equal('×');
      expect(header.element.querySelector('.terminal-name')?.textContent).to.equal('Terminal 1');
  ...
  describe('Integration Tests', () => {
    it('should complete full Claude status update flow', () => {
      ...
      const statusElement = header.element.querySelector('.claude-status');
      const indicatorElement = header.element.querySelector('.claude-indicator') as HTMLElement;
      
      expect(statusElement?.textContent).to.equal('Claude Code connected');
      expect(indicatorElement?.textContent).to.equal('●');
      expect(indicatorElement?.style.color).to.equal('#4CAF50');
    });

    it('should handle performance optimization during Claude Code session', () => {
      // Setup
      const mockTerminal = {
        write: sinon.spy(),
      } as any;
      ...
      // Act - simulate Claude Code output
      performanceManager.scheduleOutputBuffer('Claude Code response', mockTerminal);

      // Assert - should use immediate write for Claude Code mode
    ...
    it('should maintain state consistency across operations', () => {
      ...
      uiManager.updateClaudeStatusDisplay('Terminal 1', 'connected');
      uiManager.updateClaudeStatusDisplay('Terminal 2', 'disconnected');
      uiManager.updateClaudeStatusDisplay('Terminal 1', 'none');

      // Assert - check final state
      const status1 = header1.element.querySelector('.claude-status');
      const status2 = header2.element.querySelector('.claude-status');

      expect(status1).to.be.null; // Should be cleared
      expect(status2?.textContent).to.equal('Claude Code disconnected');
    });
  });
});