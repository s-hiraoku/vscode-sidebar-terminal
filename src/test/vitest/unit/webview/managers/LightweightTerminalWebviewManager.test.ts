...
vi.mock('../../../../../webview/managers/TerminalLifecycleCoordinator', () => ({
  TerminalLifecycleCoordinator: class {
    ...
    initializeSimpleTerminal = vi.fn();
...
vi.mock('../../../../../webview/managers/ConsolidatedMessageManager', () => ({
  ConsolidatedMessageManager: class {
    ...
    updatePanelLocationIfNeeded = vi.fn().mockReturnValue(false);
    getCurrentPanelLocation = vi.fn().mockReturnValue('sidebar');
    getCurrentFlexDirection = vi.fn().mockReturnValue('column');
    dispose = vi.fn();
  },
}));
...
vi.mock('../../../../../webview/managers/CliAgentStateManager', () => ({
  CliAgentStateManager: class {
    detectAgentActivity = vi.fn().mockReturnValue({ isAgentOutput: false });
...
vi.mock('../../../../../webview/managers/EventHandlerManager', () => ({
  EventHandlerManager: class {
    ...
    onPageUnload = vi.fn();
    getEventStats = vi.fn().mockReturnValue({});
    dispose = vi.fn();
  },
}));
...
describe('LightweightTerminalWebviewManager', () => {
  ...
  describe('Terminal Operations', () => {
    it('should coordinate terminal creation', async () => {
      ...
      expect(manager.postMessageToExtension).toHaveBeenCalledWith(
  ...
  describe('Panel Location Sync', () => {
    ...
    it('should switch to split mode if panel location is panel and multiple terminals exist', () => {
      ...
      (splitManager.getTerminals as any).mockReturnValue(
        new Map([
          ['t1', {}],
          ['t2', {}],
        ])
