...
describe('Terminal History Management - Regression Tests', () => {
  ...
  describe('Regression: Terminal History Processing Issues (Commit 1ead11a)', () => {
    it('should prevent infinite terminal deletion loops that caused system freeze', async () => {
      ...
      const mockTerminalManager = {
        ...
        deletionInProgress: new Set<string>(),
      ...
      expect(successfulDeletions).toHaveLength(1);
      expect(duplicateDeletions).toHaveLength(2);
      expect(mockTerminalManager.terminals.has('term-1')).toBe(false);
      expect(mockTerminalManager.deletionInProgress.size).toBe(0);
    });
    ...
    it('should handle terminal number recycling correctly without conflicts', () => {
      ...
      expect(numberManager.assignNumber()).toBeNull();
      ...
      expect([newNumber1, newNumber2]).toEqual(expect.arrayContaining([2, 4]));
      expect(numberManager.getUsedNumbers()).toHaveLength(5);
    });
    ...
    it('should preserve terminal history during session save/restore cycle', async () => {
      ...
      expect(saveResult.terminalCount).toBe(2);
      ...
      expect(restoredTerminal1).toBeDefined();
      expect(restoredTerminal2).toBeDefined();
      if (restoredTerminal1 && restoredTerminal2) {
        expect(restoredTerminal1.scrollback).toEqual(originalTerminals[0]!.scrollback);
        expect(restoredTerminal2.scrollback).toEqual(originalTerminals[1]!.scrollback);
      }
    });
  });
  ...
  describe('Regression: VS Code Standard Features Implementation (Commit d542481)', () => {
    it('should handle Alt+Click cursor positioning correctly', () => {
      ...
      expect(clickResult.handled).toBe(true);
      expect(clickResult.newPosition).toEqual({ row: 5, col: 10 });
    });
    ...
    it('should integrate with VS Code themes correctly', () => {
      ...
      expect(mockThemeManager.currentTheme).toBe('dark');
      expect(mockThemeManager.cssVariables.get('--vscode-terminal-background')).toBe('#1a1a1a');
      expect(mockThemeManager.cssVariables.get('--vscode-terminal-foreground')).toBe('#d4d4d4');
      expect(mockThemeManager.cssVariables.get('--vscode-terminal-ansiRed')).toBe('#f14c4c');

      const generatedCSS = mockThemeManager.generateCSS();
      expect(generatedCSS).toContain('--vscode-terminal-background: #1a1a1a');
      expect(generatedCSS).toContain('--vscode-terminal-foreground: #d4d4d4');
    });
  });

  describe('Regression: TypeScript Compilation and Quality Improvements (Commit 6a73e44)', () => {
    it('should prevent TypeScript compilation errors in MessageManager', async () => {
      ...
      const mockCoordinator: IManagerCoordinator = {
        sendMessageToExtension: vi.fn().mockResolvedValue(undefined),
        log: vi.fn(),
      };
      ...
      await vi.advanceTimersByTimeAsync(10); // Not strictly needed if promises resolve immediately, but safe

      // We need to wait for the microtask queue to drain since setReady calls void flushQueue()
      await new Promise(process.nextTick);
      await new Promise(process.nextTick);

      expect(mockCoordinator.sendMessageToExtension).toHaveBeenCalledTimes(2);
    });
    ...
    it('should handle WebView disposal properly to prevent memory leaks', () => {
      ...
      // Add mock resources
      const mockEmitter = { dispose: vi.fn() };
  ...
  describe('Regression: Performance and Memory Optimization', () => {
    it('should prevent buffer overflow in high-frequency terminal output', () => {
      ...
      expect(stats.totalSize).toBeLessThan(1000 * 2000); // Should not exceed max limits
    });
    ...
    it('should cleanup CLI agent detection patterns to prevent memory growth', () => {
      ...
      expect(patternManager.getPatternCount()).toBe(50);

      // Test cleanup
      patternManager.cleanup();
      expect(patternManager.getPatternCount()).toBe(0);
    });
  });
});
