...
### VS Code再起動時に実行中のプロセスが終了する

VS Code標準ターミナルとは異なり、この拡張機能ではVS Code再起動時に実行中のプロセス（Claude Code、Gemini CLI、その他の長時間実行アプリケーションなど）は終了します。
...
これはアーキテクチャ上の制限です。VS Code標準ターミナルはPTYプロセスへの再接続機能を持ちますが、この拡張機能では未実装です。

**回避策**: VS Codeの再起動をまたいでプロセスを維持する必要がある場合は、`tmux`や`screen`などのターミナルマルチプレクサを使用してください。
